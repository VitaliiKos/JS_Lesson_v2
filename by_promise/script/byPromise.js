...
function startDay(myVariables) {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            ...
            reject('Сьогодні вихідний. Можна повалятись в ліжку, почитати книжку...', '<br/>');
        }, 1000);
    })
}

//f2
function clearTheCarOfSnow(myVariables) {
...
//f3
function trafficJams(myVariables) {
    return new Promise((resolve) => {
        setTimeout(() => {
            let step = document.createElement('div');
            step.className = 'step';
...
//f4
function systemStatus(myVariables) {
    return new Promise((resolve) => {
        let step = document.createElement('div');
        step.className = 'step';
        step.innerHTML = `<h2>${myVariables.step}</h2>`;
...
//f5
function drinkCoffeeWithBoss(myVariables) {
    return new Promise((resolve) => {
        let step = document.createElement('div');
        step.className = 'step';
        step.innerHTML = `<h2>${myVariables.step}</h2>`;
...
//f7
function checkSystem(myVariables) {
    return new Promise((resolve) => {
        setTimeout(() => {
            let step = document.createElement('div');
            step.className = 'step';
...
//f8
function endOfWorkingDay(myVariables) {
    return new Promise((resolve) => {
...
//f9
function buyProducts(myVariables) {
    return new Promise((resolve) => {
        setTimeout(() => {
            let step = document.createElement('div');
            step.className = 'step';
...
day_btn.onclick = (e) => {
    ...
    startDay(myVariables)
        .then(startDayResult => {
            return clearTheCarOfSnow(startDayResult);
        })
        .then(clearTheCarOfSnowResult => {
            return trafficJams(clearTheCarOfSnowResult);
        })
        .then(trafficJamsResult => {
            return systemStatus(trafficJamsResult);
        })
        .then(systemStatusResult => {
            return drinkCoffeeWithBoss(systemStatusResult);
        })
        .then(drinkCoffeeWithBossResult => {
            return workDay(drinkCoffeeWithBossResult);
        })
        .then(workDayResult => {
            return checkSystem(workDayResult);
        })

        .then(checkSystemResult => {
            return endOfWorkingDay(checkSystemResult);
        })

        .then(endOfWorkingDayResult => {
            return buyProducts(endOfWorkingDayResult);
        })

        .then(buyProductsResult => {
            return atHome(buyProductsResult);
        })
        .then(atHomeResult => {
            ...
            wake_up.innerHTML = `<h3>Мій настрій: ${atHomeResult.mood}</h3>`;
            message.innerText = `Кошти: ${atHomeResult.money}`;
        ...
        .finally(() => {
            let wake_up = document.createElement('div');
            wake_up.className = 'final'
            let message = document.createElement("h3");
            message.innerText = `Як щвидко плине час!`;

            wake_up.appendChild(message);
            day_block.appendChild(wake_up);
        });
