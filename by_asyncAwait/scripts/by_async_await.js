...
async function myWorkDay(myVariables) {
    try {
        const step1 = await startDay(myVariables);
        const step2 = await clearTheCarOfSnow(step1);
        const step3 = await trafficJams(step2);
        const step4 = await systemStatus(step3);
        const step5 = await drinkCoffeeWithBoss(step4);
        const step6 = await workDay(step5);
        const step7 = await checkSystem(step6);
        const step8 = await endOfWorkingDay(step7);
        const step9 = await buyProducts(step8);
        const step10 = await atHome(step9);
        let result = document.createElement('div');
        result.className = 'result';
        let wake_up = document.createElement('div');
        let message = document.createElement("h3");

        wake_up.innerHTML = `<h3>Мій настрій: ${step10.mood}</h3>`;
        message.innerText = `Кошти: ${step10.money}`;
