...
// 1. Створити під кожен елемент окремий блок.
// В цьому блоці, під кожну властивість, та властивості внутрішніх об'єктів створити
// свої окремі блок.
...
for (const usersListElement of usersList) {
    ...
    explorer(usersListElement, user_card);
}
task1.appendChild(document.createElement('hr'));

// ------------------------------------------------------------------------------------------------

// 2. зробити div contenteditable ввести будь яке ціле слово.
...
let block_result = document.querySelector('.block_result');
...
function handle(event) {
    ...
    if (keyboard_key.key === 'Tab') {
        let block_text = block.textContent;
        let tag_elem = document.createElement(`${block.textContent}`)
        tag_elem.innerText = `Ви створили tag: ${block_text}`;
        block_result.appendChild(tag_elem)
    }
}

task2.appendChild(document.createElement('hr'));
// ------------------------------------------------------------------------------------------------
// 3.за допомоги рекурсії перебрати структуру сторінки.
...
function searchTags(element) {
    for (const arrElement of element) {
        if (heading_tag.includes(arrElement.localName)) {
            result.headings_arr.push(arrElement.localName);
        } else if (arrElement.localName === 'p') {
            result.paragraph_arr.push(arrElement);
...
console.log(result);
task3.appendChild(document.createElement('hr'));
